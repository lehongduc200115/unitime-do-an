...
import {
  ...
  IEngineInput,
  IEngineInputClass,
  IEngineInputInstructor,
  IEngineInputNewClass,
  IEngineInputPeriod,
  IEngineInputRoom,
  IEngineInputStudent,
  IEngineInputSubject,
...
class EngineInput implements IEngineInput {
      ...
      return {
        ...
        instructors: rowItem.instructors
          ...
          .map((instructorId: string) => {
            return this.instructors.findIndex(
              (instructor: IEngineInputInstructor) => {
  ...
  convertTimetable(input: any[]) {
    this.timetable = input.map((rowItem: any, classI: number) => {
      const instructorI = this.instructors.findIndex(
        (instructor: IEngineInputInstructor) =>
          instructor.id === rowItem.instructorId.toString()
      );
      const subjectI = this.subjects.findIndex(
        (subject: IEngineInputSubject) =>
          subject.id === rowItem.subjectId.toString()
      );
      const roomI = this.subjects.findIndex(
        (room: IEngineInputSubject) => room.id === rowItem.roomId.toString()
      );
      ...
      for (let i = minI; i <= maxI; ++i) {
        this.instructors[instructorI].activeClasses[
          EWeekday[rowItem.weekDay.toString()]
        ] =
          this.instructors[instructorI].activeClasses[
            EWeekday[rowItem.weekDay.toString()]
          ] || new Array(this.periods.length);
        this.instructors[instructorI].activeClasses[
          EWeekday[rowItem.weekDay.toString()]
        ][i] = classI;
        this.rooms[roomI].activeClasses[EWeekday[rowItem.weekDay.toString()]] =
          this.rooms[roomI].activeClasses[
            EWeekday[rowItem.weekDay.toString()]
          ] || new Array(this.periods.length);
        this.rooms[roomI].activeClasses[EWeekday[rowItem.weekDay.toString()]][
          i
  ...
  convertEnrollment(input: any[]) {
    input.forEach((rowItem: any) => {
      ...
      if (classId != null) {
        const classI = this.timetable.findIndex(
          (classItem: IEngineInputClass) => {
            return classItem.id === classId;
          }
        );
        const classItem = this.timetable[classI];
        const studentI = this.students.findIndex(
          (student: IEngineInputStudent) => {
            return student.id === studentId;
          }
        );
        // Add class to student's active classes
        for (let i = classItem.startPeriod; i <= classItem.endPeriod; ++i) {
          this.students[studentI].activeClasses[classItem.weekday] =
            this.students[studentI].activeClasses[classItem.weekday] ||
            new Array(this.periods.length);
  ...
  convertClasses(input: any[]) {
    this.newClasses = input.map((rowItem: any, newClassI: number) => {
      ...
      return {
        ...
        minEntrants: parseInt(rowItem.entrants),
        maxEntrants: parseInt(rowItem.maxEntrants),
        instructors: this.subjects.find((subject) => {
          ...
          .map((periodRange: string) => {
            return periodRange
              ...
              .map((periodId: string, index: number) => {
                return (
                  this.periods.findIndex((period: IEngineInputPeriod) => {
                    return period.id === periodId;
                  }) -
                  index * (parseInt(rowItem.period) - 1)
                );
              });
          }) ?? [[0, this.periods.length - 1]],
        preferedCampus: rowItem.preferedCampus,
        preferedRoom:
          rowItem.preferedRoom
            ?.toString()
            .split(",")
            .map((roomId: string) => {
              return this.rooms.findIndex((room: IEngineInputRoom) => {
...
const checkCapableStudent = ({
  ...
  for (
    let subjectStudentI = 0;
    subjectStudentI < maxSubjectStudentI;
    ++subjectStudentI
  ) {
    const studentI =
      engineInput.subjects[subjectI].newStudents[subjectStudentI];
    let isNotFit = false;

    for (let periodI = 0; periodI < maxPeriodI; ++periodI) {
      // Active periods
      const activeClassI =
        engineInput.students[studentI].activeClasses[weekday]?.[periodI];
      if (activeClassI != null) {
        ...
        if (
          distanceTo(engineInput.rooms[roomI], engineInput.rooms[activeRoomI]) >
          0
        ) {
          isNotFit = true;
          break;
...
const newClassEvaluate = ({
  ...
  instructorI: number;
  extendedInput: {
    // Active classes for each object, indexing: [object's index][weekday][periodI]
    rooms: number[][][];
    instructors: number[][][];
  };
  ...
  do {
    ...
    for (let preferedFrameI = 0; preferedFrameI < maxI; ++preferedFrameI) {
      if (
        engineInput.newClasses[newClassI].preferedPeriod[preferedFrameI][0] <=
          startPeriodI &&
        startPeriodI <=
          engineInput.newClasses[newClassI].preferedPeriod[preferedFrameI][1]
      ) {
        isNotFit = false;
        break;
      }
    ...
    if (newRoomI != null) {
      // If unrestricted to move old class
      for (let periodI = startPeriodI; periodI < maxI; ++periodI) {
        // Check if instructor is available
        let activeClassI =
          engineInput.instructors[instructorI].activeClasses[weekday]?.[
            periodI
          ];
        if (
          (activeClassI != null &&
          ...
          (extendedInput.instructors[instructorI]?.[weekday]?.[periodI] !=
            null &&
        ...
        activeClassI =
          engineInput.rooms[roomI].activeClasses[weekday]?.[periodI];
        if (oldClassI != null) {
          if (
            (activeClassI != null &&
              !modifiedClasses.some((modClass) => {
          ...
          oldClassI =
            activeClassI || extendedInput.rooms[roomI]?.[weekday]?.[periodI];
      ...
      if (oldClassI != null) {
        ...
        for (
          let periodI = engineInput.timetable[oldClassI].startPeriod;
          periodI < maxI;
          ++periodI
        ) {
          // Check if new room is vacant
          const activeClassI =
            engineInput.rooms[newRoomI].activeClasses[weekday]?.[periodI];
          if (
            (activeClassI != null &&
              !modifiedClasses.some((modClass) => {
                return modClass.id === engineInput.timetable[activeClassI].id;
    ...
    // Type match
    if (
      engineInput.rooms[roomI].type != engineInput.newClasses[newClassI].type
    ) {
      score = 0;
      break;
    }
    ...
    if (
      engineInput.rooms[roomI].capacity <
      engineInput.newClasses[newClassI].minEntrants
    ) {
      score = 0;
      break;
    }
    ...
    // Check if room is in preferred campus
    if (engineInput.newClasses[newClassI].preferedCampus != null) {
      if (
        engineInput.rooms[roomI].campus !==
        engineInput.newClasses[newClassI].preferedCampus
      ) {
        score = 0;
        break;
      }
      ...
      for (let i = 0; i < maxI; ++i) {
        ...
        if (proxClassI != null) {
          if (
            distanceTo(
              engineInput.rooms[roomI],
              engineInput.rooms[engineInput.timetable[proxClassI].roomI]
            ) > 0
          ) {
            isNotFit = true;
            break;
    ...
    if (
      capableStudents.length < engineInput.newClasses[newClassI].minEntrants
    ) {
      score = 0;
      break;
    } else {
      ...
      score =
        ...
        Math.floor(
          (score / 2 / maxEntrants) *
            (maxEntrants - Math.min(maxEntrants, capableStudents.length))
        );
    }
    ...
    if (newRoomI != null && oldClassI != null) {
      ...
      for (
        ...
        extendedInput.rooms[oldRoomI][weekday] =
          extendedInput.rooms[oldRoomI][weekday] ||
          new Array(engineInput.periods.length);
        extendedInput.rooms[oldRoomI][weekday][periodI] = -1;
        // Add instructor to extended input
        extendedInput.instructors[instructorI] =
          extendedInput.instructors[instructorI] || new Array(7);
        extendedInput.instructors[instructorI][weekday] =
          extendedInput.instructors[instructorI][weekday] ||
          new Array(engineInput.periods.length);
    ...
    for (let periodI = startPeriodI; periodI < maxI; ++periodI) {
      ...
      extendedInput.rooms[roomI][weekday] =
        extendedInput.rooms[roomI][weekday] ||
        new Array(engineInput.periods.length);
      extendedInput.rooms[roomI][weekday][periodI] =
        engineInput.timetable.length + newClassI;
      // Add instructor to extended input
      extendedInput.instructors[instructorI] =
        extendedInput.instructors[instructorI] || new Array(7);
      extendedInput.instructors[instructorI][weekday] =
...
const scaleupClassEvaluate = ({
  ...
  extendedInput: {
    // Active classes for each object and classes tend to be scaleup or moved, indexing: [object's index][weekday][periodI]
    rooms: number[][][];
    instructors: number[][][];
  };
  modifiedClasses: IEngineInputClass[];
  ...
  do {
    ...
    // -- Check room capacity
    const newClassEntrants =
      engineInput.newClasses[newClassI].minEntrants +
      engineInput.timetable[oldClassI].students.length;
    // Current room satisfied?
    let oldRoomI =
      modifiedClasses.find((modClass) => {
        return modClass.id === engineInput.timetable[oldClassI].id;
    ...
    if (engineInput.rooms[oldRoomI].capacity < newClassEntrants) {
      ...
      // Type check
      if (
        engineInput.rooms[newRoomI].type !==
        engineInput.newClasses[newClassI].type
      ) {
        score = 0;
        break;
      }
      ...
      for (let periodI = startPeriod; periodI <= endPeriod; ++periodI) {
        ...
        if (oldClassI_2 != null) {
          ...
          oldClassI_2 =
            activeClassI || extendedInput.rooms[newRoomI]?.[weekday]?.[periodI];
      ...
      if (oldClassI_2 != null) {
        ...
        for (
          let periodI = engineInput.timetable[oldClassI_2].startPeriod;
          periodI < maxI;
          ++periodI
        ) {
          // Check if new room is vacant
    ...
    const capableStudents = checkCapableStudent({
      ...
      periodCount:
        engineInput.timetable[oldClassI].endPeriod -
    ...
    if (oldClassI_2 != null) {
      ...
      for (let periodI = startPeriod; periodI <= endPeriod; ++periodI) {
        ...
        extendedInput.rooms[newRoomI_2] =
          extendedInput.rooms[newRoomI_2] || new Array(7);
        extendedInput.rooms[newRoomI_2][weekday] =
          extendedInput.rooms[newRoomI_2][weekday] ||
          new Array(engineInput.periods.length);
    ...
    engineResult.forEach((entity: Entity, suggestionI: number) => {
      ...
      engineInput.timetable.forEach((originClass) => {
        this.classResult[suggestionI].push({
          id: originClass.id,
          subject: `${engineInput.subjects[originClass.subjectI].id} - ${
            engineInput.subjects[originClass.subjectI].name
          }`,
          instructor: `${
            engineInput.instructors[originClass.instructorI].id
          } - ${engineInput.instructors[originClass.instructorI].name}`,
          room: engineInput.rooms[originClass.roomI].id,
          weekday: Object.keys(EWeekday).find(
            (key) => EWeekday[key] === originClass.weekday
          )!,
          period: `${engineInput.periods[originClass.startPeriod].id} - ${
            engineInput.periods[originClass.endPeriod].id
          }`,
          time: `${engineInput.periods[originClass.startPeriod].startTime} - ${
            engineInput.periods[originClass.endPeriod].endTime
          }`,
      ...
      for (let newClassI = 0; newClassI < maxI; ++newClassI) {
        ...
        if (engineInput.newClasses[newClassI].scaleupClass) {
          ...
          let oldClassI =
            entity.chromosome[geneI + 1] %
            engineInput.subjects[engineInput.newClasses[newClassI].subjectI]
              .classes.length;
          oldClassI =
            engineInput.subjects[engineInput.newClasses[newClassI].subjectI]
              .classes[oldClassI];
          ...
          let geneEval = scaleupClassEvaluate({
            ...
            newWeekday:
              extraGene %
              engineInput.newClasses[newClassI].preferedWeekday.length,
          ...
          if (geneEval > 0) {
            ...
            (refClass.id = engineInput.timetable[oldClassI].id),
              (refClass.instructor = `${
                engineInput.instructors[
                  engineInput.timetable[oldClassI].instructorI
                ].id
              } - ${
                engineInput.instructors[
                  engineInput.timetable[oldClassI].instructorI
            ...
            if (roomI) {
              refClass.type = "scaleup_modified";
            } else {
              roomI = engineInput.timetable[oldClassI].roomI;
              refClass.type = "scaleup";
            }
            refClass.room = engineInput.rooms[roomI].id;

            // Schedule
            const weekday = engineInput.timetable[oldClassI].weekday;
            const resWeekday = Object.keys(EWeekday).find(
              (key) => EWeekday[key] === weekday
            )!;
            refClass.weekday = resWeekday;
            const startPeriod: IEngineInputPeriod =
              engineInput.periods[engineInput.timetable[oldClassI].startPeriod];
            const endPeriod: IEngineInputPeriod =
              engineInput.periods[engineInput.timetable[oldClassI].endPeriod];
          ...
          const geneI = newClassI * 3;
          const roomI =
            entity.chromosome[geneI] %
            (engineInput.newClasses[newClassI].preferedRoom.length ||
              engineInput.rooms.length); // RoomI
          const classInfo = entity.chromosome[geneI + 1]; // instructorI & weekday & startperiodI
          const instructorI =
            Math.floor(classInfo / (7 * engineInput.periods.length)) %
            engineInput.newClasses[newClassI].instructors.length;
          const weekday =
            (classInfo % (7 * engineInput.periods.length)) %
            engineInput.newClasses[newClassI].preferedWeekday.length;
          const startPeriodI = Math.floor(
            (classInfo % (7 * engineInput.periods.length)) / 7
          ); // Check from preferred period range for suitability
          ...
          refClass = {
            ...
            instructor: `${
              engineInput.instructors[
                engineInput.newClasses[newClassI].instructors[instructorI]
              ].id
            } - ${
              engineInput.instructors[
                engineInput.newClasses[newClassI].instructors[instructorI]
          ...
          if (geneEval > 0) {
            ...
            // Schedule
            const resWeekday: string = Object.keys(EWeekday).find(
              (key) => EWeekday[key] === weekday
            )!;
            refClass.weekday = resWeekday;
            const startPeriod: IEngineInputPeriod =
              engineInput.periods[startPeriodI];
            const endPeriod: IEngineInputPeriod =
              engineInput.periods[
                startPeriodI + engineInput.newClasses[newClassI].period - 1
              ];
            refClass.period = `${startPeriod.id}-${endPeriod.id}`;
            refClass.time = `${startPeriod.startTime}-${endPeriod.endTime}`;
      ...
      modifiedClasses.forEach((modClass) => {
        ...
        this.classResult[suggestionI][originClassI].room =
          engineInput.rooms[modClass.roomI].id;
        this.classResult[suggestionI][originClassI].weekday = Object.keys(
          EWeekday
        ).find((key) => EWeekday[key] === modClass.weekday)!;
        const startPeriod = engineInput.periods[modClass.startPeriod];
        const endPeriod = engineInput.periods[modClass.endPeriod];
        this.classResult[suggestionI][
          originClassI
        ].period = `${startPeriod.id}-${endPeriod.id}`;
        this.classResult[suggestionI][
          originClassI
        ].time = `${startPeriod.startTime}-${endPeriod.endTime}`;
      ...
      newClasses.forEach((newClass, newClassI) => {
        if (newClass.type !== "not_available") {
          let modifiedClassI = this.classResult[suggestionI].findIndex(
            (outputClass) => {
          ...
          if (modifiedClassI < 0) {
            this.classResult[suggestionI].push({
              ...newClass,
              id: `${engineInput.timetable.length + newClassI + 1}`,
              entrants: Math.min(
                engineInput.newClasses[newClassI].maxEntrants,
                engineInput.newClasses[newClassI].minEntrants +
                  newClass.capableStudents.length
              ),
            });
          } else {
            this.classResult[suggestionI][modifiedClassI].type = "scaleup_modified";
            this.classResult[suggestionI][modifiedClassI].entrants = Math.min(
              this.classResult[suggestionI][modifiedClassI].entrants +
                newClass.capableStudents.length,
              engineInput.newClasses[newClassI].maxEntrants
            );
            this.classResult[suggestionI][modifiedClassI].capableStudents.push(
              ...newClass.capableStudents
            );
...
const fitness = (entity: Entity) => {
  ...
  for (let newClassI = 0; newClassI < maxI; ++newClassI) {
    if (engineInput.newClasses[newClassI].scaleupClass) {
      ...
      let oldClassI =
        entity.chromosome[geneI + 1] %
        engineInput.subjects[engineInput.newClasses[newClassI].subjectI].classes
          .length;
      oldClassI =
        engineInput.subjects[engineInput.newClasses[newClassI].subjectI]
          .classes[oldClassI];
      ...
      const startPeriodI = Math.floor(
        (classInfo % (7 * engineInput.periods.length)) / 7
      ); // Raw iteration; modulus to max possible length of each prefered period range for usable iteration
      const newRoomI = entity.chromosome[geneI + 2];
  ...
  for (let i = 0; i < topResultCount; ++i) {
    const dump = (_: any, loop: any) => {
      if (loop % 1000 == 0) {
        console.log(
          `${
            (topResultCount - i - 1) * engineClassConfig.generation + loop
          } iteration left`
        );
      }
    };
